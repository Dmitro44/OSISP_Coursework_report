...
// #show text: it => {
//   box(align(center + horizon, skew(ax: -17deg, it)))
// }
...
#let doc(code, name-lines, note) = {
  for (i, line) in name-lines.enumerate() {
    ...
    ([], code-cell, left-cell[#line], note-cell)
  }
}
...
#let footerTable(config) = {
  text(size: 10pt, table(
    ...
    cell(colspan: 2, left-footer-cell[Разраб.]),
    left-footer-cell[#config.developer],
    [],
    [],
    cell(rowspan: 5, text(size: 12pt, [#upper(config.title.join(linebreak())) \ #text(size: 12pt)[#config.doc-type]])),
    cell(colspan: 3)[Лит.],
    [Лист],
    [Листов],
    //row

    cell(colspan: 2, left-footer-cell[Пров.]),
    left-footer-cell[#config.reviewer],
    [],
    [],
    [],
    [#config.lit],
    ...
    [],
    [],
    cell(colspan: 5, rowspan: 3, text(size: 12pt, [#config.department \ группа #config.group])),
    //row
