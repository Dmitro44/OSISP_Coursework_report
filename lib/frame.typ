...
#let frame(type:"a", paper:"a4",flipped:false, config: default-config, stroke:1pt, font:"Arial", show_label: false, position: "right", content) = {
  ...
  let main-caption(config) = if type == "а" or type == "a"{
    ...
    cell(colspan: 7, rowspan: 3,  [#text(size: 16pt)[#config.doc-number]]),
    ...
    [Изм.], [Лист], [№ докум.], [Подп.], [Дата], cell(rowspan:3)[],  cell(rowspan:3)[#config.lit], cell(rowspan:3)[], cell(rowspan:3, colspan:2)[], cell(rowspan:3)[],

    cell(colspan: 2, role-cell[Разраб.]), role-cell[#config.developer], [], [],

    cell(colspan: 2, role-cell[Пров.]), role-cell[#config.reviewer], [], [],

    cell(colspan: 2, role-cell[]), [], [], [], cell(colspan:4)[ Лист #config.current-page], cell(colspan:2)[Листов #config.total-pages],

    cell(colspan: 2, role-cell[Н.контр.]), role-cell[#config.norm-control],[],[], cell(rowspan:3)[], cell(colspan: 6, rowspan: 3,  [#text(size: 12pt)[#config.department-group]]),
